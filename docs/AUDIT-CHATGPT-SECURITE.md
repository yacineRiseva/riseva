...
- [ ] **1. Le schéma ne se déploie pas.** `mission.realise`, `mission.date_mission`,
...
- [ ] **2. La couche Supabase n'existe pas.** `connecterSupabase()` ajoute
...
- [ ] **3. Fonctions privilégiées exposées.** `anonymiser_salarie()` et
...
- [ ] **4. Escalade immédiate en administrateur Riseva.** La policy `UPDATE` sur
...
- [ ] **5. Score et état de mission falsifiables.** Le salarié peut insérer
...
- [ ] **6. Fuite intra-entreprise.** Tous les salariés lisent les missions de tous
...
- [ ] **7. Toutes les colonnes `entreprise` sont publiques** : CA, coût journalier,
...
- [ ] **8. Le filtrage par domaine n'est pas un contrôle.** `domaine_autorise()`
...
- [ ] **9. Quota contournable et soumis à une course.** Deux inscriptions
...
- [ ] **10. Un salarié retiré reste autorisé.** `mon_role()` et `mon_entreprise()`
...
- [ ] Jeton d'invitation faible : quatre caractères, ~19,8 bits, `random()` non
  cryptographique, oracle public. → `gen_random_bytes(16)`, stockage du SHA-256 seul.
- [ ] Une association non validée ou suspendue peut publier. → Création par RPC.
...
- [x] Points synthétiques de démonstration (`p.retenu || e.points`) qui
  ...
  et sur le salarié.*
- [ ] Trois classements incompatibles (vue SQL brute, vue matérialisée toutes
  saisons, JS synthétique). → Un seul agrégat `(entreprise, saison, type)`.
- [ ] Dénominateur manipulable : l'admin peut réduire `effectif` à 1.
...
- [ ] Deux moteurs automatiques concurrents (`02` et `05`). → Un seul.
- [ ] Rapports scellés à la fin du trimestre alors que les validations restent
...
- [ ] Reçus fiscaux : `recu_numero` libre, sans unicité ; commentaire encore en
  16216*01 alors que le JS est en 16216*03.
- [ ] **L'« anonymisation » n'en est pas une** : même UUID conservé partout,
...
- [ ] Aucune purge : `conservation_ans` n'a aucun effet technique.
  → `purge_le`, `legal_hold`, `tache_retention()` quotidienne, journal de purge.

## P2 — contraintes, index, échelle

- [ ] Contraintes : `annonce.restant <= quantite`, unité et multiplicateur tous
...
- [ ] `points_pour()` doit lever si le barème manque : renvoyer zéro masque une
  configuration cassée.
- [ ] `ON DELETE CASCADE` sur saisons, associations et annonces → `RESTRICT`.
- [ ] Index manquants : `profil(entreprise, role)`, `mission(declaree_le) where
...
- [ ] `classement_saison` rescanne `mission` par entreprise. → Agrégat set-based.
- [ ] `ca integer` déborde au-delà de 2 147 483 647 €. → `numeric(15,2)`.
